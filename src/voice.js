...
export function normalizeVoiceMode(value) {
  ...
  if (!["windows", "file", "heygen"].includes(mode)) throw new Error("VOICE_MODE debe ser windows, file o heygen");
  return mode;
}
...
export async function resolveVoiceFile(value) {
  ...
  if (![".mp3", ".wav", ".m4a"].includes(path.extname(file).toLowerCase())) {
    throw new Error("El archivo de voz debe ser .mp3, .wav o .m4a");
  }
